...
pub fn process_borrow(ctx: Context<Borrow>, amount: u64) -> Result<()> {
    ...
    match ctx.accounts.mint.to_account_info().key() {
        key if key == user.usdc_address => {
            ...
            let sol_accrued_interest = calculate_accrued_interest(user.deposited_sol, bank.interest_rate, user.last_updated)?;
        ...
        _ => {
            ...
            total_collateral = usdc_price.price as u64 * user.deposited_usdc; // no interest in usdc
        }
